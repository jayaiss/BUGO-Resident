...
/* -----------------------------
   Page URL base
----------------------------- */
$baseUrl  = enc_page('audit'); // e.g. "index_Admin.php?page=ENCRYPTED..."
...
?>
<h3><i class="fa fa-clipboard-list"></i> Audit Logs List</h3>
...
<?php
  ...
  if (!isset($pageBase)) {
      $baseUrl      = enc_page('audit');                 // e.g. index_Admin.php?page=ENC...
      $basePath     = strtok($baseUrl, '?');             // index_Admin.php
      $baseQueryStr = (string)parse_url($baseUrl, PHP_URL_QUERY);
      parse_str($baseQueryStr, $baseQueryArr);
